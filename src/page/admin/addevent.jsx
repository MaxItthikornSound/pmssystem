...
import { add_event } from '../component/connectdatabase';
...
const Addevent = () => {
    ...
    const checkvaluenull = (value, element) => value ? document.getElementById('inputgroups_' + element).style.border = '1px solid rgb(222, 226, 230)' : document.getElementById('inputgroups_' + element).style.border = '2px solid red';
    ...
    const selectonchange = (event) => { 
        ...
        setEvaluate(event); 
        event ? element[0].style.border = '1px solid rgb(222, 226, 230)' : element[0].style.border = '2px solid red';
    }
    ...
    const save_event = async () => {
        ...
        if (!topic.value || !description.value || !startdate.value || !enddate.value) {
            ...
            if (location.state) {
                if (location.state.filter(item => item.EventStatusDate === StatusDate && item.EventEvaluate === Evaluate.value).length) {
                    alertsmall('warning', 'This evaluation round is already here, please select the correct evaluator or date.');
                } else {
                    const result_addevent = await add_event(topic.value, Evaluate.value, description.value, startdate.value, enddate.value, StatusDate);
                    result_addevent === 'addevent_success' ? alertsuccessredirect('Add Event Success', '/manageevent') : null;
                }
            } else {
                const result_addevent = await add_event(topic.value, Evaluate.value, description.value, startdate.value, enddate.value, StatusDate);
                result_addevent === 'addevent_success' ? alertsuccessredirect('Add Event Success', '/manageevent') : null;
                                ...
                                <Col md={12} className='inputgroups_yellow' style={{height: '400px', backgroundColor: 'white', borderRadius: '25px'}}>
                                                    ...
                                                    <Form onSubmit={(e) => e.preventDefault()}>
                                                        <Form.Group>
                                                            ...
                                                            <InputGroup id='inputgroups_topic' className='inputgroups_eval'>
                                                                <Form.Control type='text' id='topic' className='inputtext' onChange={txtonchange} placeholder='Topic' />
                                                            </InputGroup>
                                                        </Form.Group>
                                                    </Form>
                                                </Col>
                                            ...
                                            <Form onSubmit={(e) => e.preventDefault()}>
                                                <Form.Group>
                                                    ...
                                                    <InputGroup id='inputgroups_description' className='inputgroups_eval'>
                                                        <Form.Control as='textarea' id='description' rows={3} onChange={txtonchange} placeholder='Description' />
                                                    </InputGroup>
                                                </Form.Group>
                                            </Form>
                                        </Col>
                                    ...
                                    <Row className='midpoint' style={{height: '100px', marginTop: '40px'}}>
                                                    ...
                                                    <Form onSubmit={(e) => e.preventDefault()}>
                                                        <Form.Group>
                                                            ...
                                                            <InputGroup id='inputgroups_startdate' className='inputgroups_eval'>
                                                                <Form.Control type='date' id='startdate' className='inputtext' onChange={txtonchange} placeholder='Start Date' />
                                                            </InputGroup>
                                                        </Form.Group>
                                                    </Form>
                                                </Col>
                                        ...
                                        <Col md={3} className='midpoint' style={{marginTop: '30px'}}>
                                            <Button variant='warning' className='btns' style={{fontSize: '14px'}} onClick={save_event}>Save</Button>
                                        </Col>
                                    </Row>
                                </Col>
                            </Row>
